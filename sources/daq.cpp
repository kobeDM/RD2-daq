...
int main(int carg, char **szarg){
    if(carg != 8){
        ...
        std::cout << "./daq Frequency(Hz) Trigger_level(V) rise/fall Trigger_position(s) DynamicRange(V) Entries File_name_prefix" << std::endl;
        std::cout << "i.e., ./daq 20000000.0 -1.0 rise 0.0 2.0 100 test1" << std::endl;
        return 0;
    }
    // Configuration
    ...
    const int ENTRIES = std::stoi(szarg[6]); // Number of Entries in one subrun
    const char* FILE_NAME_PREFIX = szarg[7];
    const double CHANNEL_RANGE = std::stod(szarg[5]); // V
    ...
    struct timeval _time_auto;
    gettimeofday(&_time_auto, NULL);
	...
	// Calc rate
	if(iTrigger % 1 == 0 && iTrigger != 0){
		double diff = double(sec - time_to_calc_rate);
		if (diff < 1) diff = 1./9999.;
		ofs << "Rate(Hz) " << iTrigger << " " << sec << " " << 1./diff << std::endl;
